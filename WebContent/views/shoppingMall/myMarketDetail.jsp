...
<script type="text/javascript">
	function btn_click(str){
		if(str == 'cart'){
			frm1.action = "CartProcess.do";
		}else if(str == 'buy'){
			frm1.action = "OrderSelect.do";
		}	
	}
	function checkValue()
    {
        inputForm = eval("document.frm1");
        if(!inputForm.amount.value)
        {
            alert("수량을 입력하세요");    
            inputForm.amount.focus();
            return false;
        }
    }
</script>
...
<div align="center">
	...
	<table border="0">
		...
		<tr align = "center">
			<td width= "200">판매자</td>
			<%String a = dto.getgCommenter(); %>
			
			
			<% 
				out.println("<td>");
							out.println("<a href = 'views/shoppingMall/UserInfo.jsp?id=" + a +"'>" + dto.getgCommenter());
				out.println("</td>");
			%>
		...
		<tr align = "center">
			...
			<td>
				<%
					...
					out.println("<input type='number'min='1'name = 'amount' max= " + dto.getgAmount() +" style='width: 500px; height:30px;' />");
				}
				%>
			</td>			
		</tr>
		...
		<tr align = "center">
			<td colspan = "2" >
				<!-- script에서 user_pwd에 접근하기 위한 폼, 폼에서 Enter키 입력으로 인한 화면 전환 방지 -->
					...
					<input type='submit' value='장바구니 담기' onClick='btn_click("cart");' style="width: 120px; height:40px;" />
					<input type='submit' value='주문하기' onClick='btn_click("buy");' style="width: 120px; height:40px;"/>
